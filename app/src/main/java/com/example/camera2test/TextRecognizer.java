...
//                    }
        ...
        currentCount++;
        if (textList.size() > mGraphicOverlay.totalWords && mGraphicOverlay.totalWords != 0) { //TODO: This Is creating problems while reading as sometimes when the TTS is speaking it deletes the textList Which Cause Problems/
            // So i have to create a condition which only clears the text after its done speaking.
            textList.clear();
        }

//        if (mGraphicOverlay.graphicsBox.size() > mGraphicOverlay.totalGraphics){ // This Prevents the graphics from exceding too much
